...
class ThumbnailWorker(QObject):
    ...
    def run(self):  # executed in thread
        ...
        if duration <= 0:
            ...
            return
        # adapt thumbnail count to width
        if self._width_hint > 0:
            # aim for ~100px per thumb
            approx = max(3, int(self._width_hint / 100))
...
def format_time(seconds: float) -> str:
    ...
    Rounds to milliseconds. Accepts negative for completeness (will clamp to 0 display).
    """
    if seconds < 0:
        seconds = 0.0
